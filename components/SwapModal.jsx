...
export default function SwapModal({ open, onClose, mealTitle }) {
  if (!open) return null;
      ...
      <ul className="space-y-2 text-sm">
        {items.map((it, i) => (
          <li key={i} className="flex items-start justify-between gap-3">
            <div>
              <div className="font-medium">{it.name} <span className="text-gray-500">({it.grams}غ)</span></div>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl p-5 space-y-4" onClick={(e)=>e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-green-700">بدائل الوجبة {mealTitle ? `— ${mealTitle}` : ""}</h3>
          <button onClick={onClose} className="text-gray-600 hover:text-black">إغلاق ✕</button>
        </div>
        <p className="text-xs text-gray-500 -mt-2">بدائل مكافئة تقريبًا لنفس السعرات/الماكروز. يمكنك اختيار الأنسب لك.</p>
